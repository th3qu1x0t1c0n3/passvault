...
import {getAccountById, updateAccount} from "../../service/VaultService";
...
function UpdateAccount({user}: IAccountFormProps) {
    ...
    useEffect(() => {
        ...
        getAccountById(idAccount as string).then((response) => {
    ...
    function handlePass() {
        setSubmitting(false);
        const signInObj: IsignIn = {username: user.username, password: masterPassword};
